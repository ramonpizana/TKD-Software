...
/// Saves capabilities in the build output directory so desktop builds avoid writing into
/// repo-local schema folders that can be blocked on Windows.
fn save_capabilities(out_dir: &Path, capabilities: &BTreeMap<String, Capability>) -> Result<PathBuf> {
  fs::create_dir_all(out_dir)?;
  let path = out_dir.join(CAPABILITIES_FILE_NAME);
  let json = serde_json::to_string(&capabilities)?;
  write_if_changed(&path, json)
    .with_context(|| format!("failed to write capabilities file to {:?}", path))?;

  Ok(path)
}

/// Saves ACL manifests in the build output directory so desktop builds avoid writing into
/// repo-local schema folders that can be blocked on Windows.
fn save_acl_manifests(out_dir: &Path, acl_manifests: &BTreeMap<String, Manifest>) -> Result<PathBuf> {
  fs::create_dir_all(out_dir)?;
  let path = out_dir.join(ACL_MANIFESTS_FILE_NAME);
  let json = serde_json::to_string(&acl_manifests)?;
  write_if_changed(&path, json)
    .with_context(|| format!("failed to write ACL manifests file to {:?}", path))?;
...
fn inline_plugins(
  ...
  for (name, plugin) in inlined_plugins {
    // Windows rejects `:` in directory names, but plugin identifiers may still contain it.
    let plugin_fs_name = name.replace(':', "-");
    let plugin_out_dir = out_dir.join("plugins").join(&plugin_fs_name);
    fs::create_dir_all(&plugin_out_dir).with_context(|| {
      format!(
        "failed to create inline plugin output directory {:?} for plugin {}",
        plugin_out_dir, name
      )
    })?;
...
"###
      ...
      .with_context(|| format!("failed to define autogenerated inline plugin permissions for {name}"))?
    };
    ...
    if let Some(pattern) = plugin.permissions_path_pattern {
      ...
      .with_context(|| format!("failed to define custom inline plugin permissions for {name}"))?);
    } else {
      let default_permissions_path = Path::new("permissions").join(&plugin_fs_name);
      ...
      .with_context(|| format!("failed to load filesystem inline plugin permissions for {name}"))?);
    }
...
fn app_manifest_permissions(
  ...
  fs::create_dir_all(&app_out_dir)
    .with_context(|| format!("failed to create app manifest output directory {:?}", app_out_dir))?;
  ...
  let mut permission_files = if manifest.commands.is_empty() {
    ...
    .context("failed to define autogenerated app permissions")?
  };
  ...
  if let Some(pattern) = manifest.permissions_path_pattern {
    ...
    .context("failed to define custom app permissions")?);
  } else {
    ...
    let inlined_plugins_permissions: Vec<_> = inlined_plugins
      .keys()
      .map(|name| permissions_root.join(name.replace(':', "-")))
    ...
    .context("failed to load default app permissions from ./permissions")?);
  }
...
pub fn build(out_dir: &Path, _target: Target, attributes: &Attributes) -> super::Result<()> {
  let mut acl_manifests = read_plugins_manifests().context("failed to read plugin manifests")?;
  ...
  .context("failed to prepare app ACL")?;
  ...
  let inline_plugins_acl = inline_plugins(out_dir, attributes.inlined_plugins.clone())
    .context("failed to prepare inline plugin ACL")?;

  acl_manifests.extend(inline_plugins_acl.manifests);

  let _acl_manifests_path =
    save_acl_manifests(out_dir, &acl_manifests).context("failed to save ACL manifests")?;
  ...
  let capabilities = if let Some(pattern) = attributes.capabilities_path_pattern {
    tauri_utils::acl::build::parse_capabilities(pattern)
      .with_context(|| format!("failed to parse capabilities from pattern {pattern}"))?
  } else {
    println!("cargo:rerun-if-changed=capabilities");
    tauri_utils::acl::build::parse_capabilities("./capabilities/**/*")
      .context("failed to parse capabilities from ./capabilities/**/*")?
  };
  validate_capabilities(&acl_manifests, &capabilities).context("failed to validate capabilities")?;

  let _capabilities_path =
    save_capabilities(out_dir, &capabilities).context("failed to save capabilities")?;
  ...
  tauri_utils::acl::build::generate_allowed_commands(out_dir, Some(capabilities), permissions_map)
    .context("failed to generate allowed commands")?;
